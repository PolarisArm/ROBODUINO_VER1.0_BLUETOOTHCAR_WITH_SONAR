...
AMotorController::AMotorController(void) {
    TimerInitalized = false;
}

void AMotorController::enable(void) {
  // setup the latch
  /*
  LATCH_DDR |= _BV(LATCH);
...
void AMotorController::latch_tx(void) {
  uint8_t i;
...
A_DCMotor::A_DCMotor(uint8_t num, uint8_t freq) {
  motornum = num;
  pwmfreq = freq;
...
void A_DCMotor::run(uint8_t cmd) {
  uint8_t a, b;
  switch (motornum) {
  case 1:
...
void A_DCMotor::setSpeed(uint8_t speed) {
  switch (motornum) {
  case 1:
    setPWM1(speed); break;
  case 2:
...
A_Stepper::A_Stepper(uint16_t steps, uint8_t num) {
  MC.enable();
...
void A_Stepper::setSpeed(uint16_t rpm) {
  usperstep = 60000000 / ((uint32_t)revsteps * (uint32_t)rpm);
  steppingcounter = 0;
}

void A_Stepper::release(void) {
  if (steppernum == 1) {
    latch_state &= ~_BV(MOTOR1_A) & ~_BV(MOTOR1_B) &
      ~_BV(MOTOR2_A) & ~_BV(MOTOR2_B); // all motor pins to 0
    MC.latch_tx();
...
void A_Stepper::step(uint16_t steps, uint8_t dir,  uint8_t style) {
...
uint8_t A_Stepper::onestep(uint8_t dir, uint8_t style) {
  uint8_t a, b, c, d;
  uint8_t ocrb, ocra;
